...
| 6 | **Nx'ni qachon kiritish** — egasining qarori (2026-07-25): avval poydevorni o'rganish, shuning uchun `apps/admin` mustaqil Vite loyihasi (o'z package.json'i bilan), API esa repo root'ida qoladi; Nx + `apps/api` ko'chirish + `libs/shared` keyinroq. Ungacha Zod sxemalar FE tomonda nusxalanadi (drift xavfi qabul qilingan, kichik saqlanadi)<br>**Hozirgacha to'plangan dalil (2026-08-05):** `PHONE_PATTERN` uchta faylda yashaydi — `src/users/dto/user-fields.schema.ts` va FE'dagi profil hamda xodim forma sxemalari — va uchalasini birdan tahrirlashga to'g'ri keldi. Aynan shunday takrorlanishni `libs/shared` yo'q qiladi.<br>**Nx foydasiga dalil _bo'lmaydigan_ narsa:** takrorlangan *ishga tushirish paytidagi qiymatlar*. Harakatsizlik chegarasi `REFRESH_TOKEN_TTL` ning ikkinchi nusxasi edi; u API login/refresh javobida `sessionIdleMs` jo'natishi bilan hal qilindi (FRONTEND.md §B.10) — bu doimiyni baham ko'rishdan yaxshiroq, chunki qayta qurish talab qilmaydi. Faqat *kompilyatsiya paytidagi kontraktlar* — sxemalar, DTO turlari, enumlar — haqiqatan `libs/shared` ga muhtoj | Umumiy-kontrakt og'rig'i yoki ikkinchi iste'molchi ilova paydo bo'lganda |
| 7 | **Har bir davlat uchun telefon validatsiyasi (`libphonenumber-js`)** — egasining qarori (2026-08-04): hozir telefon oddiy kontakt maydoni, shuning uchun E.164 *shakl* tekshiruvi (`/^\+?\d{7,15}$/`, 22 ta davlatda tasdiqlangan) mutanosib; ~90–145 KB metadata esa yo'q. Qabul qilingan bo'shliq: o'z davlati uchun noto'g'ri uzunlikdagi raqam baribir o'tadi | SMS-xabarnoma joriy qilinganda (Faza 5, buyurtmalar) — o'shanda yetib bormagan raqam puldan ketadi, shunchaki tartibsizlik emas |
| 8 | **Sessiyalar ro'yxatida qurilma yorlig'i (iPad + takroriy parser)** — egasining qarori (2026-08-07): keyinga qoldirildi. Sessiyalar kartasi har bir jonli sessiyani faqat `User-Agent` sarlavhasidan nomlaydi. Ikkita nuqson, ikki xil egasi: **(a) bizniki emas** — iPadOS 13+ Safari ataylab `Macintosh; Intel Mac OS X` deb ko'rsatadi, shuning uchun iPad "Safari · macOS" bo'lib, noutbuk ikonkasi bilan chiqadi. Buni hech qanday regex tuzatolmaydi: UA qatorida bu ma'lumot chindan ham yo'q. Faqat client yuboradigan `navigator.maxTouchPoints` ishorasi hal qiladi, va faqat shuning uchunki Apple hech qachon sensorli ekranli Mac chiqarmagan — bu `Macintosh` + sensor kombinatsiyasini ikkilanishsiz qiladi (xuddi shu qoida Windows'ga qo'llansa, sensorli noutbuklarda noto'g'ri ishlaydi). **(b) bizniki** — `parseUserAgent` (`src/shared/utils/user-agent.util.ts`) va `deviceInfo` (`apps/admin/src/features/profile/lib.ts`) deyarli bir xil parserlar bo'lib, aynan bir xil xatoni tashiydi; ya'ni har qanday UA tuzatishi ikki marta qilinishi kerak. `device_type` allaqachon `varchar(10)`, shuning uchun `'tablet'` migratsiya talab qilmaydi.<br>**Nega bu kosmetika emas:** sessiyalar kartasi — xavfsizlik ekrani, uning yagona vazifasi egaga "bu qurilmani tanimayapman" degan xulosaga kelish imkonini berish. Doimiy noto'g'ri yorliq uni nomuvofiqliklarni e'tiborsiz qoldirishga o'rgatadi — bu ekranni foydasizdan ham battar qiladi.<br>**Narx bahosi (2026-08-07):** ~30 daqiqa, migratsiya yo'q, yangi paket yo'q | Sexda chindan planshet ishlatilganda yoki responsive shell (UI-F) bilan birga — qaysi biri oldin kelsa. Client-hint infratuzilmasi (axios default sarlavhasi) o'sha ish bilan umumiy |
